---
description: Analyzes newly written or modified functions in `code.gs` and generates isolated, idempotent unit tests in `tests.gs`.
---

# Execution Steps
1. **Analyze Code Delta:** Review the most recent additions or modifications to `code.gs`.
...
5. **Draft & Patch:** Generate the test functions using the `Assert` library and append them to `tests.gs`. Output a brief summary of the test coverage to the chat.